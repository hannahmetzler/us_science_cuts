...
# Expand to one row per email, keeping all grant info
...
cat("Unique grants:", n_distinct(nsf_emails$grant_id), "\n")
cat("Unique emails:", n_distinct(nsf_emails$email), "\n")
cat("Cut grant rows:", sum(nsf_emails$was_cut), "\n")

# To get unique email list for sending survey:
# nsf_emails %>% distinct(email, .keep_all = TRUE)
#
# To filter by directorate first:
# nsf_emails %>% filter(directorate == "SBE") %>% distinct(email, .keep_all = TRUE)
```
