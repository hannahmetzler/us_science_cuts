...
# To load the end product of this script, load nih_joined here:
# nih_joined <- readRDS(here('data_analysis/nih_analysis.rds'))
...
## Combine NIH Exporter data from 2014-2024 with 2025 data - load nih_exporter_usa data from cache

```{r combine-manual-with-exporter-load_nih_exporter_usa}
...
saveRDS(nih_joined, here('data_analysis/nih_analysis.rds'))
cat("NIH analysis dataset:", nrow(nih_joined), "grants,", ncol(nih_joined), "columns\n")
cat("Saved to data_analysis/nih_analysis.csv and .rds\n")
```
...
| Total columns  | 46      |
| Cut grants     | 5,814   |
| Non-cut grants | 224,642 |
| Fiscal years   | 2014-2025 |

## Column Structure (46 columns)

**From ExPORTER (27 columns)** - authoritative grant data:

- IDs: application_id, core_project_num, full_project_num
- Agency: administering_ic, study_section, funding_mechanism
- PI: pi_ids, pi_names
- Organization: org_name, org_city, org_state, org_zipcode, org_country, org_district, org_dept, ed_inst_type
- Project: project_title, project_terms, phr, nih_spending_cats, abstract_text
- Dates: fiscal_year, project_start, project_end
- Budget: total_cost, direct_cost_amt, indirect_cost_amt

**From Grant Witness (18 columns)** - termination-specific:

- Status: status, ever_frozen
- Dates: frozen_date, unfrozen_date, termination_date, targeted_start_date, targeted_end_date
...
- was_cut: TRUE/FALSE flag indicating if grant was terminated/frozen