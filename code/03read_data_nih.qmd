...
sapply(nih_exporter_list, ncol)
```

```{r combine-datasets}
# Combine all years into one dataset
# First check if columns are consistent

# Get column names from each year
col_names_list <- lapply(nih_exporter_list, names)
...
# Combine using only common columns for safety
nih_exporter_all <- bind_rows(
  lapply(nih_exporter_list, function(df) select(df, all_of(common_cols)))
)
...
exporter_cols <- c(
  ...
  # Metadata
  "ACTIVITY", "FUNDING_MECHANISM", "STUDY_SECTION"
)
...
cat("Recommended ExPORTER columns:", length(exporter_cols), "\n")
cat("Recommended Grant Witness columns:", length(grantwitness_cols), "\n")
```
